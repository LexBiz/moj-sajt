...
export function buildTemoWebFirstMessage() {
  // Requirement: first message across ALL channels is a fixed intro (UA).
  // Default language remains Ukrainian unless the user asks otherwise.
  return [
    ...
    '— 1) Розкажіть про бізнес і звідки зараз приходять заявки',
    '— 2) Опишіть, що саме хочете автоматизувати (повідомлення/ліди/запис/оплата)',
    '— 3) Напишіть, які канали потрібні (Instagram / WhatsApp / Messenger / сайт)',
...
export function applyNextSteps(params: {
  ...
  if (lines.length < 3) {
    if (stage === 'DISCOVERY') {
      ...
      lines.push(
        lang === 'ua'
          ? 'Уточню, який канал для старту пріоритетний і чи потрібна інтеграція з CRM.'
          : 'Уточню, какой канал для старта приоритетный и нужна ли интеграция с CRM.',
      )
    } else if (stage === 'TRUST') {
      ...
      lines.push(lang === 'ua' ? 'Скажу, що потрібно від вас, щоб стартувати без затримок.' : 'Скажу, что нужно от вас, чтобы стартовать без задержек.')
    } else {
      ...
      lines.push(
        lang === 'ua'
          ? 'Поясню, що потрібно підготувати, щоб запустити все за 48–72 години.'
          : 'Объясню, что нужно подготовить, чтобы запустить всё за 48–72 часа.',
      )
    }
  }
