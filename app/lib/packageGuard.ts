...
export function ensureAllPackagesMentioned(text: string, lang: 'ru' | 'ua') {
  ...
  const helper =
    lang === 'ua'
      ? 'Скажіть 2 речі: скільки каналів стартуємо і чи потрібні оплати/календар/CRM — і я чітко порекомендую 1 пакет та план запуску.'
      : 'Скажите 2 вещи: сколько каналов запускаем и нужны ли оплаты/календарь/CRM — и я чётко порекомендую 1 пакет и план запуска.'
  return `${text}\n\n${header}\n${lines.join('\n')}\n${helper}`.trim()
}
